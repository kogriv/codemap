...
- ~~**A debounced file watcher**~~ — **closed 2026-08-28 (M3.2)**, and this разбор is why: CodeGraph shipped
  the loop codemap had deferred, which turned "we are waiting for a live scenario" into "we are waiting because
  we never priced it". codemap now ships `codemap watch` + `serve --watch`. **Both sides then measured the same
  way** (second pass — the first comparison put our end-to-end against their *manual sync*, which was
  apples-to-oranges in both directions): save → answerable is **327–424 ms** for CodeGraph with its watcher
  running, **8.1–8.7 s** for codemap at the defaults, of which 4.3 s is the fast-tier rebuild. **~20–25×**, not
  the ~70× the manual number implied. Their edge is partly a Rust kernel and partly a design worth stealing:
  the debounce is **adaptive** (≤2 pending files → 300 ms quiet window, bursts keep the full 2 s), where
  codemap shipped a flat 2 s → backlog **M3.2-f1**. ([CodeGraph card](tools/codegraph.md))
