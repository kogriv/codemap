...
- [ ] **R1-C41 манифест скоупа описывает не тот набор файлов, из которого построен граф** (S) — 🆕
      (2026-09-02, заведено второй мишенью, [issue #15](https://github.com/kogriv/codemap/issues/15), рядом
      ...
      **Scope:** D1 — в git-режиме добавить `git ls-files --others --exclude-standard` (ровно тот набор, что
      ...
      падение сборки. **Открыто:** бампать ли схему — рекомендация нет (поле аддитивное, формой как `skipped`).
      **Приёмка:** неотслеживаемый модуль попадает в манифест, двигает `scope_id` и пересчитывается
      инкрементальной сборкой (сегодня проваливаются все четыре); игнорируемый назван и диагностикой, и
      `provenance.inputs` у графа, прочитанного без сайдкара; на чистых сборках `codemap` и
      `codemap --consumer tests --docs docs` — 0 нарушений (сегодняшняя база: 0 из 51 и 0 из 164);
      закреплённый бенчмарк R2 `sha256:300e0a01…5e47d2` проверен явно и не двигается; откат строки
      перечисления красит тест (R1-C37).
      **Бьёт ли сейчас:** нет — игнорируемых/неотслеживаемых `.py` внутри анализируемых корней сегодня 0 и у
      нас, и на догфуд-мишени. Дефект латентный, и открывается он в самом обычном состоянии дерева — когда
      модуль создан и ещё не добавлен в индекс.
