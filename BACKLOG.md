...
- [x] **R1-C25 провенанс графа: артефакт не говорит, кто его собрал, из чего и когда он перестал быть правдой** ✅
      ...
      **провенанс — не схема**; схема описывает форму файла, процесс не описывает ничто. `codemap diff` на
      ...
      настоящего эпизода (у P5 «провенанс» — про другое: роли мульти-рута, это соседство R1-C24).
...
**Релизный workflow намеренно не добавлен.** Публикация сделана руками — один раз, по явному указанию.
Триггер по тегу с trusted publisher — правильная форма для *второго* релиза (воспроизводимо, и токен
нигде не лежит), но это решение о том, как владелец хочет релизить, а не то, что стоит ставить за него
попутно с первым выпуском.
