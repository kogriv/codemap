...
- [~] **R1-C43 инкрементальная deep-сборка консервирует шум и слепнет от него** 🟡 (S сделано, M открыто) —
      ...
      **Не замерено (честная граница):** систематически ли частичный пересчёт слабее полного — состояние
      `13/0` не достигнуто 9 раз из 9 при 4 из 8 у полной сборки, но прогон 12×12 прерван; числа нет —
      значит и утверждения нет.
