...
- [~] **M3.2 Свежесть** — 🟡 **три из четырёх кирпичей на месте**, остался только опциональный авто-watcher.
      ...
      **R1-C9** (2026-08-23) — быстрый инкремент (`build --incremental`, deep ~5s vs ~60s); **issue #3**
      ...
      отдельный пункт не нужно. **Переоценка вверх (2026-08-28, R2/CodeGraph):** критерий «ждём живого
      сценария» держался на том, что цена петли неизвестна. Теперь известна — у пира она измерена:
      нативные файловые события → debounce 2 с → инкрементальная досборка, **121 мс** на один изменённый файл,
      и новый символ сразу отвечает на запрос. Все три наших кирпича на месте, недостающее — glue.
...
- [ ] **R1-C28 лимит — это тоже партиальность, и мы про неё молчим** (S) — 🆕 (2026-08-28, из разбора
...
- [x] **R1-C25 провенанс графа: артефакт не говорит, кто его собрал, из чего и когда он перестал быть правдой** ✅
      ...
      **провенанс — не схема**; схема описывает форму файла, процесс не описывает ничто. `codemap diff` на
      ...
      настоящего эпизода (у P6 «провенанс» — про другое: роли мульти-рута, это соседство R1-C24).
