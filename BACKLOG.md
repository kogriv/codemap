...
- [ ] **R1-C22 `high`-полоса dead-code врёт в 39% (ссылки, которые видны в исходнике, но не в графе)** 🆕
      (2026-08-24, **схема не нужна**) — из issue [#7](https://github.com/kogriv/codemap/issues/7), найдено
      ...
      **Приёмка — НЕ байт-идентичность** (в отличие от R1-C21: здесь мы намеренно добавляем истинные рёбра):
      ...
      (**D1–D5 открыты**). **Смыкается с** R1-C8 (градация), R1-C20 (тот же выбор: моделировать связь, а не
      чинить отчёт), R1-C13.
