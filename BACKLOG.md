...
**Открытые решения:** O1 `scope_id` в graph.json? (реком. нет) · O2 материализация staging vs
манифест-поверх-дерева (реком. материализация) · O3 sha-256 full (реком.) · O4 JSON-spec (реком.) ·
O5 включать не-код `.md` (реком. да) · **O6 git-биндинг — ✅ утверждён** (git: перечисление/провенанс/diff;
идентичность = sha256). O1–O5 — ждут «ок». Детали — в дизайн-доке §5.
