...
**Статус:** ✅ **M0 + M1 + M1.5 + M4 + M5 + M2 сделаны** (M2 — 2026-07-26, виды RAG/vault/mermaid).
**В работе — M6** (repo scope / impact: мульти-рут ядро+потребители+доки, режимы thin/full) —
следствие обкатки видов 2026-07-28 (`gaps/observability_dogfood_2026-07-28.md`, F1).
Осталось после — **M3** (serve/свежесть: тёплый режим, freshness-check, SQLite при масштабе).
...
codemap видит только пакет `bquant/`, а на вопрос «кто использует / можно ли удалить X» blast
radius живёт в тестах/доках/examples/scripts. Расширяем область до **мульти-рута в пределах
одного репо** (§10.12), сохраняя нейтральное ядро. **Ценность — не «ещё файлов», а транзитивный
типизированный impact-анализ, которого grep не даёт.** Схема → `0.4` (аддитивно). Оба режима
настоящие — прогнать и **сравнить эмпирически** (обкатка-2 на том же `MACDZoneAnalyzer`).

- [ ] **M6.1 Роуты + провенанс** — `extract` принимает core-роут + N consumer-роутов + doc-роуты;
      каждый узел помечается `extras.root` (`core`/`tests`/`examples`/`research`/`scripts`/`docs`).
      Манифест роутов (`codemap.toml`) или CLI-флаги (§10.12).
- [ ] **M6.2 Режим thin** — потребители индексируются только на **исходящие ссылки в ядро**
      (`imports`/`calls`/`inherits` к `core.*`); внутренняя структура не материализуется. Дёшево.
- [ ] **M6.3 Режим full** — все роуты на равную глубину (griffe по каждому); все узлы/рёбра.
      Флаг `--mode thin|full`; дефолт — thin (дёшево/CI), full доступен.
- [ ] **M6.4 Doc-слой** — `extract/md_extractor.py`: скан `*.md` на `from core… import X` и
      точечные `core.a.b.C` → `doc`-узлы + `references`-рёбра к символам ядра (переиспользуя логику
      doc-parity). Тогда «удаление X ударит по N докам» видно в графе.
- [ ] **M6.5 Inbound-refs в query (F2)** — `query <symbol>` выводит входящие ссылки (кто зовёт/
      импортирует/наследует/ссылается), **сгруппированные по роуту и типу ребра**. Blast-radius наружу.
- [ ] **M6.6 Impact-вид** — `report impact <symbol>` / `export`: транзитивный blast radius
      (что упадёт прямо + что косвенно), с разбивкой по роутам и меткой `resolution` (честность).
- [ ] **M6.7 Тесты + обкатка-2** — фикстур с core+consumer+doc роутами; на bquant — `MACDZoneAnalyzer`
      теперь отдаёт полный blast radius (тесты+доки), сверка с grep; замер thin vs full (узлы/рёбра/время).

**DoD:** на bquant impact-запрос по `MACDZoneAnalyzer` возвращает backward-compat тест-набор +
doc-ссылки (то, что grep нашёл вручную), сгруппированные по роуту и типу ребра, транзитивно;
thin/full сравнены; детерминизм держится; схема `0.4`. Findings F3 (class-neighbors в RAG) и
F4 (registry-map вид) — кандидаты, не блокеры M6.
