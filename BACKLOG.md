...
- [ ] **M19.A codemap-фича: манифест входа** (M) — `codemap/scope.py` (резолв+хэш+профиль+`scope_id`);
      CLI `codemap scope <path> [--consumer/--docs/--spec] [--json]` (резолв без сборки) и
      `codemap scope --diff <a.meta> <b.meta>`; `build` пишет блок `scope` в M18-сайдкар. `graph.json`
      остаётся чисто структурным (без схемы). Тесты: детерминизм, флип одного хэша, `--diff`, excludes.
      **Субстрат для отложенного:** контент-хэши = вход Merkle для **R1-C9** (пересчёт только изменённого) и
      hash-свежести для **M3.2** (точнее mtime). Три пункта сходятся сюда.
- [ ] **R2.0.1 харнесс бенч-скоупа** (S) — см. ниже, в R2 (реализуется поверх M19.A).

**Открытые решения (на утверждение):** O1 `scope_id` в graph.json? (реком. нет) · O2 материализация staging
vs манифест-поверх-дерева (реком. материализация) · O3 sha-256 full (реком.) · O4 JSON-spec (реком.) ·
O5 включать не-код `.md` (реком. да). Детали — в дизайн-доке §5.
...
- [ ] **R2.0.1 Харнесс бенч-скоупа** (S) — 🟡 дизайн ([docs/design/scope.md](docs/design/scope.md) §3).
      Единый источник правды для входа бенча → каждая карточка доказуемо сравнима. `research/tools/_scope/
      bquant.scope.json` (spec: 6 каталогов, venv исключён) + `materialize.py` (материализует чистое дерево +
      зовёт `codemap scope` из M19.A → `manifest.json` с `scope_id`). Шаблон карточки и `comparison.md`
      получают поле **Scope = scope_id + профиль**; все hands-on-строки обязаны нести один `scope_id`.
      Ретро-проставить `scope_id` в карточку graphlens. **Зависит от M19.A.**
