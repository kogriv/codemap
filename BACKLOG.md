...
**Не сделано намеренно (D7): сама публикация.** Упаковка готова и проверена, но выгрузка — необратимое
внешнее действие (занятое имя на PyPI навсегда, номер версии не переиспользуется), и это триггер владельца.
Релизного workflow и trusted-publisher не добавляю на догадках о том, как он хочет это делать.
