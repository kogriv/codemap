...
**Не сделано намеренно (D7): публикация.** Имя дистрибутива не выбрано (`codemap` на PyPI занято), а имя —
дверь в одну сторону: команда установки, совет по импорту в каждой доке, URL. Всё в этой вехе — предпосылка
публикации, и ничто не предрешает ответ. Релизного workflow, trusted-publisher и триггера по тегу нет.
